...
class SVMScikit:
    def __init__(self) -> None:
        self.regr = MultiOutputRegressor(SVR(C=5, epsilon=0.15, kernel='poly'))
...
def load_dataset(dataset_name: str = 'students', 
    ...
    # dataset more than 10000 we sample only 100000
    if X.shape[0] > 100000:
...
def one_hot_encoding_system(x_system):
    ...
    tr_x_system[:, 
                col_st+F_NAMES['mtype']:col_st+F_NAMES['val_acc']+1] =\
                    x_system[:, F_NAMES['mtype']:F_NAMES['val_acc']+1]
...
def generate_system(x_system):
  model = None
  (mtype, kernel, C, deg, gamma, coef, tol, _, _) = tuple(x_system)
  ...
  # SVC
  if mtype == 0:
    model = feature_set(SVC)
  else:
     model = feature_set(NuSVC)  
...
def ass_accuracies(y_pred, y_pred_error, y_true_label, ep):
    # the system
    y_pred_label = np.argmax(y_pred, axis=1)
    not_corrects = y_pred_label != y_true_label
    ass_found = np.abs(y_pred_error[:, y_pred_label]) > ep

    return np.mean(ass_found == not_corrects)
...
def train_assessor(X, 
    ...
    y_one_hot = np.zeros((n_train, n_labels), dtype=int)
    y_one_hot[:, y] = 1
    ...
    def updaete(epochs=1):
        for i, model in enumerate(systems):
            ...
            y_pred = model.predict_proba(X[cond])
            y_ass_train[cond] = y_pred
            x_ass_train[cond] = x_system[i]

        y_ass_delta = y_one_hot - y_ass_train
